...
$("#postTextarea, #replyTextarea").keyup((event) => {
    ...
    if(submitButton.length == 0) return alert("No submit button found"); //alert() for development purposes
...
$("#userSearchTextbox").keydown((event) => {
    ...
    //event.keyCode == 8 is backspace/delete key (also keyCode is deprecated)
...
$("#deletePostButton").click((event) => {
    ...
    //This code only deletes the postId
    //If a retweet was made that document will not be deleted
    $.ajax({
        url: `/api/posts/${postId}`,
        type: "DELETE",
        success: (postData) => {
...
//Attaches click handeler to document
//Necessary as <div class='post'> is dynamic content
...
$(document).on("click", ".followButton", (e) => {
    ...
    $.ajax({
        ...
        success: (data, status, xhr) => { 
            ...
            //Check followers for profile page
...
$(document).on("click", ".notification.active", (e) => {
    ...
    e.preventDefault(); //Redirect user to href AFTER markAsOpened runs
...
function getPostIdFromElement (element)
{
    ...
    if(postId === undefined) return alert ("postId undefined"); //alert() for development
...
function messageTime(current, sentAt)
{
    //Will format sentAt time for messages as HH:ii:ss, rather than using timeDifference()
}
...
function outputSelectableUsers(results, container) 
{
    ...
    results.forEach(result => {
        ...
        //Stops us OR already selected users being shown in search results
...
{
    //If .chatContainer instances = 0 then user not on chatPage
